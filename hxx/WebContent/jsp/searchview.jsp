...
<div class="index_banner">
      ...
      查找结果
  	 ...
  	 <div class="sub_box clearfix" id="videoBox_new">
		 ...
		 <c:forEach items="${search.list}" var="search"> 
		 <c:if test="${search.rPass==1}">
			<tr>
            <td><a href="${pageContext.request.contextPath}/findResourceByrId.action?rId=${search.rId}">${search.rName}</a></td>
            	<td>${search.rCreatetime}</td>
            <td>${search.lable.lLableName } </td>
            </tr>
            </c:if>
            </c:forEach>
		</table>
